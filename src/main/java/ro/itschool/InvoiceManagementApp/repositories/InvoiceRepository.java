...
//    List<InvoiceEntity> searchInvoicesByClient(@Param("searchTerm") String searchTerm);
    @Query("SELECT i FROM InvoiceEntity i WHERE i.client.name LIKE %:searchTerm% OR i.client.email LIKE %:searchTerm% OR i.client.city.name LIKE %:searchTerm% OR i.client.county.name LIKE %:searchTerm% OR i.client.housingType LIKE %:searchTerm%")
    List<InvoiceEntity> searchInvoicesByClient(@Param("searchTerm") String searchTerm);
